...
static struct pci_slot_attribute hotplug_slot_attr_available_images = {
	.attr = {.name = "available_images", .mode = S_IFREG | S_IRUGO},
	.show = available_images_read_file,
};

static ssize_t image_reload_write_file(struct pci_slot *pci_slot, const char *buf,
			       size_t count)
{
	struct hotplug_slot *slot = pci_slot->hotplug;
	int retval = 0;
...
static struct pci_slot_attribute hotplug_slot_attr_image_reload = {
	.attr = {.name = "image_reload", .mode = S_IFREG | S_IRUGO | S_IWUSR},
