...
/*
 * DFL device driver for PMCI private feature on Intel Arrow Creek Card
 *
			   ...
			   struct mfd_cell *cells, int n_cell)
{
	...
	for (i = 0; i < n_cell; i++) {
		if (!strcmp(cells[i].name, "ac_pmci_secure")) {
...
static int pmci_probe(struct dfl_device *ddev)
{
	...
	pmci->regmap =devm_regmap_init_indirect_register(dev,
			pmci->base + PMCI_SPI_BASE_OFF, &pmci_max10_cfg);
	if (IS_ERR(pmci->regmap))
		return PTR_ERR(pmci->regmap);

	ddata->regmap = pmci->regmap;
...
#define FME_FEATURE_ID_AC_PMCI	0xd

static const struct dfl_device_id pmci_ids[] = {
	{ FME_ID, FME_FEATURE_ID_AC_PMCI },
	{ }
};
...
MODULE_DESCRIPTION("Driver for PMCI private feature on Intel Arrow Creek Card");
