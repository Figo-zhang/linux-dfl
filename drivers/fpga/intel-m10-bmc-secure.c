...
static enum fpga_sec_err rsu_check_idle(struct m10bmc_sec *sec)
{
	...
	ret = m10bmc_sys_read(sec->m10bmc, doorbell_offset(sec->m10bmc), &doorbell);
	if (ret)
...
static int rsu_check_complete(struct m10bmc_sec *sec, u32 *doorbell)
{
	...
	if (m10bmc_sys_read(sec->m10bmc, doorbell_offset(sec->m10bmc), doorbell))
		return -EIO;
...
static u64 m10bmc_sec_hw_errinfo(struct fpga_sec_mgr *smgr)
{
	...
	case FPGA_SEC_ERR_WEAROUT:
		m10bmc_sys_read(sec->m10bmc, doorbell_offset(sec->m10bmc), &doorbell);
