...
static int
pmci_set_flash_host_mux(struct intel_m10bmc *m10bmc, bool request)
{
	int ret;
	u32 ctrl;
	       ...
	       u32 addr, u32 size)
{
	...
	ret = pmci_set_flash_host_mux(m10bmc, true);
	if (ret)
		return ret;
	...
	return pmci_set_flash_host_mux(m10bmc, false);

read_fail:
	pmci_set_flash_host_mux(m10bmc, false);
	return ret;
}
...
#define fpga_sec_bulk_read(sec, buf, addr, size)\
	((M10_N3000((sec)->m10bmc) || M10_D5005((sec)->m10bmc)) ? \
	 (m10bmc_bulk_read(sec, buf, addr, size)) : \
...
static enum fpga_sec_err rsu_update_init(struct m10bmc_sec *sec)
{
	...
	if (M10_N3000(sec->m10bmc) || M10_D5005(sec->m10bmc)) {
...
m10bmc_sec_write_blk(struct fpga_sec_mgr *smgr, u32 offset, u32 size)
{
	...
	if (sec->m10bmc->flash_ops &&
	    sec->m10bmc->flash_ops->write_blk) {
		ret = sec->m10bmc->flash_ops->write_blk(sec->m10bmc,
