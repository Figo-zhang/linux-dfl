...
m10bmc_sec_read_blk(struct m10bmc_sec *sec, void *buffer,
		u32 addr, u32 size)
{
...
static enum fpga_sec_err
pmci_sec_read_blk(struct m10bmc_sec *sec, void *buffer,
		u32 addr, u32 size)
{
	int ret;
...
#define fpga_sec_read_blk(sec, buf, addr, size)\
		(M10_SPI(sec->m10bmc) ? (m10bmc_sec_read_blk(sec, buf, addr, size)) : \
		 (pmci_sec_read_blk(sec, buf, addr, size)))
		     ...
		     u32 prog_addr, u32 reh_addr, char *buf)
{
	...
	ret = fpga_sec_read_blk(sec, hash, reh_addr, sha_num_bytes);
	if (ret) {
...
show_canceled_csk(struct device *dev, u32 addr, char *buf)
{
	...
	ret = fpga_sec_read_blk(sec, csk_le32, addr, size);
	if (ret) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = fpga_sec_read_blk(sec, flash_buf,
...
static u64 m10bmc_sec_hw_errinfo(struct fpga_sec_mgr *smgr)
{
	...
	case FPGA_SEC_ERR_WEAROUT:
		m10bmc_sys_read(sec->m10bmc, doorbell_offset(sec->m10bmc) +
				M10BMC_DOORBELL, &doorbell);
		m10bmc_sys_read(sec->m10bmc, doorbell_offset(sec->m10bmc) +
				M10BMC_AUTH_RESULT, &auth_result);
