...
#define TOD_ADJUST_MS			(((TOD_PERIOD_MAX >> 16) + 1) * (TOD_ADJUST_COUNT_MAX + 1))
...
static int coarse_adjust_tod_clock(struct dfl_tod *dt, s64 delta)
{
	...
	/*
	 * The ToD IP requires the reverse order of write to keep the timestamp is corrected
	 * when last write occurs.
	 */
