...
#define TOD_ADJUST_MS			(((TOD_PERIOD_MAX >> 16) + 1) * \
...
static int coarse_adjust_tod_clock(struct dfl_tod *dt, s64 delta)
{
	 ...
	 * The timestamp is corrected when the TOD_NANOSEC register is written.
	 */
...
static int dfl_tod_probe(struct dfl_device *ddev)
{
	...
	if (IS_ERR_OR_NULL(dt->ptp_clock))
		dev_err_probe(dt->dev, PTR_ERR_OR_ZERO (dt->ptp_clock),
