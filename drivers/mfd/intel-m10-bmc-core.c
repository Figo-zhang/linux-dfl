...
{
	struct intel_m10bmc *ddata = &mdev->m10bmc;
	...
	ret = devm_mfd_add_devices(mdev->dev, PLATFORM_DEVID_AUTO, cells, n_cell,
				   NULL, 0, NULL);
	if (ret)
		dev_err(mdev->dev, "Failed to register sub-devices: %d\n", ret);
