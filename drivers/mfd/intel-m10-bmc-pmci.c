...
static void pmci_write_fifo(struct pmci_device *pmci,
			    unsigned int reg, void *buf, size_t count)
{
	memcpy_toio(pmci->base + reg, buf, count);
}

static void pmci_read_fifo(struct pmci_device *pmci,
			   unsigned int reg, void *buf, size_t count)
{
	memcpy_fromio(buf, pmci->base + reg, count);
}
		     ...
		     u32 addr, u32 size)
{
	...
	while (size) {
		...
		pmci_read_fifo(pmci, PMCI_FLASH_FIFO, buf, blk_size);
		size -= blk_size;
		offset += blk_size;
	}
